...
public class TestDefaultMemoryMarketManager {
	...
	public void testInStock() throws Exception {
		...
		quantity = this.marketManager.getQuantity(MarketUtils.MARKET_ASDA, MarketUtils.FOOD_ID_APPLE);
		assertTrue("Default market manager did not update apple stock in ASDA correctly", quantity == 0);
		
		String NEW_ITEM = "ABC";
		FoodItem newFoodItem = new FoodItem();
		newFoodItem.setId(NEW_ITEM);
		newFoodItem.setDescription("new");
		newFoodItem.setType("TYPE");
		this.marketManager.addFoodItem(newFoodItem);
		this.marketManager.addOrUpdate(MarketUtils.MARKET_ASDA, NEW_ITEM, 10);
		quantity = this.marketManager.getQuantity(MarketUtils.MARKET_ASDA, NEW_ITEM);
		
		assertTrue("Default market manager should have the new item added in ASDA supermarket", quantity > 0);
